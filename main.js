document.addEventListener("DOMContentLoaded", function () {
  console.log("WebOS version 0.9 loaded successfully");
  ...
  function getAppContent(appName) {
    if (appName === "settings") {
      ...
      `;
    } else if (appName === "cyber-teams") {
      return `<div id="cyber-teams-container"></div>`;
    }
    // Other app contents...
  ...
  function initializeDesktop() {
    ...
    // אתחל את שאר הרכיבים של WebOS
    updateClock();
    setInterval(updateClock, 1000);
    createDayClockIcon();
    createDayClockWindow();
    updateDayClockTheme();
    setInterval(updateDayClockTheme, 60000);

    // הוסף כפתור התנתקות לתפריט ההתחלה
    addLogoutButton();
  }

  function addLogoutButton() {
    const logoutButton = document.createElement("div");
    logoutButton.className = "start-menu-item";
    logoutButton.innerHTML = '<i class="fas fa-sign-out-alt"></i> התנתק';
    logoutButton.addEventListener("click", () => {
      window.auth.logout();
      document.getElementById("desktop").style.display = "none";
      document.getElementById("login-screen").classList.add("active");
    });
    document.querySelector(".start-menu-category").appendChild(logoutButton);
  }

  function loadCyberTeamsApp() {
    const script = document.createElement("script");
    script.src = "apps/cyber-teams/cyber-teams.js";
    script.onload = () => {
      const cyberTeams = new CyberTeams();
      document.getElementById("cyber-teams-container").innerHTML =
        cyberTeams.render();
    };
    ...
    style.href = "apps/cyber-teams/cyber-teams.css";
